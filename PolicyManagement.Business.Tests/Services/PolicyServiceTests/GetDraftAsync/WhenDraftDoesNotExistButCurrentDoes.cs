...
{
    [TestFixture]
    public class WhenDraftDoesNotExistButCurrentDoes : PolicyServiceTestBase
    {
        ...
        {
            _fileShare.Verify(s => s.ReadAsync(It.Is<string>(f => f == "current/policy.json"), It.Is<CancellationToken>(f => f == Token)), Times.Once);
        }
