...
{
    [TestFixture]
    public class WhenIdMatchesCurrent : PolicyServiceTestBase
    {
        private Guid _input;
        ...
        {
            _fileShare.Verify(s => s.ExistsAsync(It.Is<string>(f => f == "current/policy.json"), It.Is<CancellationToken>(f => f == Token)), Times.Once);
        }

        [Test]
        public void Policy_Downloaded()
        {
            _fileShare.Verify(s => s.DownloadAsync(It.Is<string>(f => f == "current/policy.json"), It.Is<CancellationToken>(f => f == Token)));
        }
