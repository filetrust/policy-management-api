...
{
    [TestFixture]
    public class WhenIdMatchesDraft : PolicyServiceTestBase
    {
        private Guid _input;
        ...
        {
            _fileShare.Verify(s => s.ExistsAsync(It.Is<string>(f => f == "draft/policy.json"), It.Is<CancellationToken>(f => f == Token)), Times.Once);
        }
