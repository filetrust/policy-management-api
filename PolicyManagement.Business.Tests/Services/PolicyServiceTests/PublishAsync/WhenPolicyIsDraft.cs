...
namespace PolicyManagement.Business.Tests.Services.PolicyServiceTests.PublishAsync
{
    ...
    public class WhenPolicyIsDraft : PolicyServiceTestBase
    {
        ...
        {
            _fileShare.Verify(x => x.DownloadAsync(It.Is<string>(f => f == "current/policy.json"), It.Is<CancellationToken>(f => f == Token)), Times.Exactly(2));
        }
        ...
        {
            _fileShare.Verify(x => x.DownloadAsync(It.Is<string>(f => f == "draft/policy.json"), It.Is<CancellationToken>(f => f == Token)), Times.Once);
        }
        ...
        {
            _fileShare.Verify(x => x.DeleteDirectoryAsync(It.Is<string>(f => f == "draft"), It.Is<CancellationToken>(f => f == Token)), Times.Once);
        }
